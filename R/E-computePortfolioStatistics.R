...
#'
#' @export
computePortfolioStatistics <- function(
 linearReturnMatrix
, weightMatrix
...
, `VaR 95` = PerformanceAnalytics::VaR(R = portfolioMatrix, p = 0.95, method = "historical")
, `VaR 99` = PerformanceAnalytics::VaR(R = portfolioMatrix, p = 0.99, method = "historical")
, `ETL 95` = PerformanceAnalytics::ETL(R = portfolioMatrix, p = 0.95, method = "historical")
, `ETL 99` = PerformanceAnalytics::ETL(R = portfolioMatrix, p = 0.99, method = "historical")
, `Worst Loss` = if(ncol(linearReturnMatrix) > 1){t(data.frame(apply(portfolioMatrix,2,function(x){x<-x[!is.na(x)]; return(min(x))})))} else {min(linearReturnMatrix)}
